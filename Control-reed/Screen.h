...
int colorForLED, colorForBoxpart;
int angle_Wii = 0;
int beacon_Wii = 0;
int camera_Wii = 0;
int box=0;
      ...
      switch (receivedPack[1])
      {
        case COLORPICKER:
          switch(receivedPack[2])
          {
            case 0:
            if(color != dataIn){
            state = 6; // color over ride number
            color = dataIn;
            }
            
            break;
            case 1:   
//            robot.ToSend(MACRO_COMMAND_RECEIVE, -1);
//            robot.sendData(NAVIGATION_ADDRESS);         
//            colorForBoxpart= dataIn;            
//            robot.ToSend(LED_STATE_OVERRIDE, COLOR_BY_BOX); 
//            robot.ToSend(BOX_LOCATION, box);     
//            robot.ToSend(COLOR_PICKER, colorForBoxpart);
        ...
        case BUTTON:
          ...
          switch (receivedPack[2])
          {
            ...
            case 17:
              ...
              macro_sub_command = (180 * camera_Wii) + angle_Wii;
              break;
            case 78:    //LOCALIZE
              macro_command_screen = MACRO_SNIPPIT;
              macro_sub_command = 4;
            ...
            case 79:  //CONTINOUS
              ...
              robot.ToSend(WII_SUBSYSTEM_MODE, WIIMODE_CONTINUOUS);
              robot.sendData(PIC_ADDRESS);
              break;
            case 80: //Do HORIZONTAL
              robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
              robot.ToSend(WII_SUBSYSTEM_MODE, WIIMODE_HORIZONTAL_BEACON);
              robot.sendData(PIC_ADDRESS);
              break;
            case 81: //VERTICAL
              robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
              robot.ToSend(WII_SUBSYSTEM_MODE, WIIMODE_VERTICAL_BEACON);
              robot.sendData(PIC_ADDRESS);
              break;
            ...
            case 86://motor box
              ...
              break;
            case 45:
            case 87:// return to normal    
              state = 3; //manual operation
              color = 0; //no color          
//              robot.ToSend(LED_STATE_OVERRIDE, 3);
//              robot.ToSend(COLOR_PICKER, 0);
//              robot.ToSend(LED_UPDATE_FLAG, 1);
//              robot.sendData(LED_ADDRESS);
//              robot.ToSend(MACRO_COMMAND_RECEIVE, 0);
//              robot.sendData(NAVIGATION_ADDRESS);            
              break;
            case 88://core
              box=2;            
              ...
              //SERVO_OVERRIDE PAGE BUTTONS
              case 91://-10L
                    if(overrideAngleLeft>10) overrideAngleLeft-=10;
                    robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
                    robot.ToSend(SERVO_OVERRIDE_LEFT, overrideAngleLeft);
                    robot.sendData(PIC_ADDRESS);
              break;
              case 92://+10L
                    if(overrideAngleLeft<180) overrideAngleLeft+=10;
                    robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
                    robot.ToSend(SERVO_OVERRIDE_LEFT, overrideAngleLeft);
                    robot.sendData(PIC_ADDRESS);
              ...
              break;
              case 93://90L
                    overrideAngleLeft=90;
                    robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
                    robot.ToSend(SERVO_OVERRIDE_LEFT, 90);
                    robot.sendData(PIC_ADDRESS);              
              break;
              case 94://-10R              
               if(overrideAngleRight>10) overrideAngleRight-=10;
                    robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
                    robot.ToSend(SERVO_OVERRIDE_RIGHT, overrideAngleRight);
                    robot.sendData(PIC_ADDRESS);
              break;
              case 95://90R              
                    overrideAngleRight=90;
                    robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
                    robot.ToSend(SERVO_OVERRIDE_RIGHT, 90);
                    robot.sendData(PIC_ADDRESS);
              break;
              case 96://+10R
               if(overrideAngleRight<180) overrideAngleRight+=10;
                    robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
                    robot.ToSend(SERVO_OVERRIDE_RIGHT, overrideAngleRight);
                    robot.sendData(PIC_ADDRESS);
              break;
          ...
          switch (receivedPack[2])
          {
            ...
            case 1:
              ...
              case 2:
                ...
                {
                    robot.ToSend(LAST_BOARD_ADDRESS_RECEIVE, CONTROL_ADDRESS);
                    robot.ToSend(WII_SUBSYSTEM_MODE, WIIMODE_SERVO_OVERRIDE);
                    robot.sendData(PIC_ADDRESS);
                }
                else
                {
